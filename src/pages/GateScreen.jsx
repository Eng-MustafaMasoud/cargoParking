...
import { Car, Users, Clock, Wifi, WifiOff } from 'lucide-react'
...
const GateScreen = () => {
  ...
  const { data: gate } = useQuery({
    ...
    queryFn: () => apiClient.getGates().then(gates => gates.find(g => g.id === gateId)),
  })
  ...
  const { data: subscription } = useQuery({
    ...
    enabled: !!subscriptionId && activeTab === 'subscriber',
  })
  ...
  const canSelectZone = (zone) => {
    ...
    if (activeTab === 'subscriber') {
      return subscription?.active && 
             subscription?.allowedCategories?.includes(zone.categoryId) &&
            ...
            {subscription && (
              <div className="mt-2 p-3 bg-green-50 border border-green-200 rounded-lg">
                <p className="text-sm text-green-800">
                  ✓ Subscription active for {subscription.cars?.length || 0} car(s)
                </p>
              </div>
            )}
          </div>
